...
def clearDataBase(con):
    ...
    tableList = ["article_favorites", "article_tags",
                 "follows", "articles", "users", "tags"]
...
def fillArticles(con):
    ...
    userIDList = getColumn(con, "users", "id")
...
def fillArticleTag(con):
    ...
    articleIDList = getColumn(con, "articles", "id")
    tagIDList = getColumn(con, "tags", "id")
    ...
    for articleID in articleIDList:
        ...
        for tagID in tagIDList:
            strExecute = createStrExecuteInsert(
                "article_tags", {"article_id": articleID, "tag_id": tagID})
            cur.execute(strExecute)
...
def fillFavorites(con):
    ...
    for userID in userIDList:
        ...
        for articleID in randArticleIDList:
            strExecute = createStrExecuteInsert(
                "article_favorites", {"user_id": userID, "article_id": articleID})
            cur.execute(strExecute)
    ...
    for followed in userIDList:
        ...
        for follower in randFollowerList:
            if(followed == follower):
                continue
            strExecute = createStrExecuteInsert(
                "follows", {"follow_id": followed, "user_id": follower})
            cur.execute(strExecute)
...
def main():
    ...
    try:
        ...
        conn_string = "host=localhost port=5432 dbname=\'" + \
            sys.argv[1] + "\' user=admin password=admin"
